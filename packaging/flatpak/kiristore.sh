#!/bin/bash
# Kiri Store Flatpak launcher script
# Launches the uber JAR with the bundled JetBrains Runtime

export JAVA_HOME=/app/jre

# Ensure config directory exists
mkdir -p "${XDG_CONFIG_HOME:-$HOME/.config}/kiristore"
mkdir -p "${XDG_DATA_HOME:-$HOME/.local/share}/kiristore"
...
exec /app/jre/bin/java \
    ...
    -Dswing.aatext=true \
    -Djava.util.prefs.userRoot="${XDG_CONFIG_HOME:-$HOME/.config}/kiristore" \
    -Dapp.data.dir="${XDG_DATA_HOME:-$HOME/.local/share}/kiristore" \
    -Dapp.downloads.dir="${XDG_DOWNLOAD_DIR:-$HOME/Downloads}" \
    -jar /app/lib/kiristore.jar "$@"

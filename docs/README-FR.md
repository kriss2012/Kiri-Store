...
Consultez le [Wiki](https://github.com/OpenHub-Store/GitHub-Store/wiki) de Kiri Store pour la FAQ et des informations utiles
...
Kiri Store est un projet open source indépendant, non affilié à GitHub, Inc.  
...
> [!IMPORTANT]
> **Utilisateurs macOS :** Il est possible qu'un avertissement indique qu'Apple ne peut pas vérifier Kiri Store. Cela est dû au fait que l'application est distribuée hors de l'App Store et n'est pas encore notariée. Autorisez-la via Réglages Système → Confidentialité et sécurité → Ouvrir quand même.
...
- **Gestion des applications**
    - Ouvrez, désinstallez et rétrogradez les applications installées directement depuis Kiri Store.
...
## 🔍 Comment mon application apparaît-elle dans Kiri Store ?

Kiri Store n'utilise aucune indexation privée ni règle de curation manuelle.  
...
2. **Fichiers installables dans la dernière release**
    ...
    - Kiri Store ignore les archives de code source générées automatiquement (`Source code (zip)` / `Source code (tar.gz)`).
...
Si votre dépôt remplit ces conditions, Kiri Store peut le trouver via la recherche et l'afficher automatiquement — aucune soumission manuelle n'est requise.

---

## ✅ Avantages / Pourquoi utiliser Kiri Store ?
...
- **Sait ce que vous avez installé**
  Suit les applications installées via Kiri Store (Android) et signale les nouvelles releases disponibles, pour que vous puissiez les mettre à jour sans retourner chercher sur GitHub.
...
## 🔐 Certificat de Signature APK de Kiri Store

Toutes les releases officielles de Kiri Store sont signées avec l'empreinte de certificat suivante :
...
| Champ                          | Valeur                                        |
| ------------------------------ | --------------------------------------------- |
| **Application name**           | Ce que vous voulez (ex. *Kiri Store Dev*)   |
...
Kiri Store est développé et maintenu par un lycéen. Votre soutien l'aide à :
...
Kiri Store vous aide uniquement à découvrir et télécharger des fichiers de releases déjà publiés sur GitHub par des développeurs tiers.  
...
En utilisant Kiri Store, vous comprenez et acceptez que vous installez et exécutez tout logiciel téléchargé à vos propres risques.  
...
Kiri Store sera publié sous la **Licence Apache, Version 2.0**.

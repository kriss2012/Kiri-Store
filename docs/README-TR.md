...
SSS ve yararlı bilgiler için Kiri Store [Wiki](https://github.com/OpenHub-Store/GitHub-Store/wiki) sayfasına göz atın
...
Kiri Store, GitHub, Inc. ile bağlantısı olmayan bağımsız bir açık kaynak projesidir.  
...
> [!IMPORTANT]
> **macOS Kullanıcıları:** Apple'ın Kiri Store'u doğrulayamadığını belirten bir uyarı görebilirsiniz. Bu, uygulamanın App Store dışında dağıtılması ve henüz notarize edilmemiş olması nedeniyle gerçekleşir. Sistem Ayarları → Gizlilik ve Güvenlik → Yine de Aç yolunu izleyerek izin verin.
...
- **Uygulama yönetimi**
    - Kurulu uygulamaları doğrudan Kiri Store üzerinden açın, kaldırın ve sürüm düşürün.
...
## 🔍 Uygulamam Kiri Store'da nasıl görünür?

Kiri Store herhangi bir özel indeksleme ya da manuel kürasyon kuralı kullanmaz.  
...
2. **En son sürümde kurulabilir dosyalar**
    ...
    - Kiri Store, GitHub'ın otomatik oluşturduğu kaynak arşivlerini (`Source code (zip)` / `Source code (tar.gz)`) yoksayar.
...
Deponuz bu koşulları sağlıyorsa Kiri Store onu arama yoluyla bulabilir ve otomatik olarak gösterebilir — manuel gönderim gerekmez.

---

## ✅ Artılar / Kiri Store neden kullanılır?
...
- **Ne kurduğunuzu bilir**
  Kiri Store (Android) üzerinden kurulan uygulamaları takip eder ve yeni sürümler mevcut olduğunda vurgular; böylece GitHub'da tekrar arama yapmadan güncelleyebilirsiniz.
...
## 🔐 Kiri Store APK İmzalama Sertifikası

Tüm resmi Kiri Store sürümleri şu sertifika parmak iziyle imzalanmıştır:
...
| Alan                           | Değer                                           |
| ------------------------------ | ----------------------------------------------- |
| **Application name**           | İstediğiniz bir şey (örn. *Kiri Store Dev*)   |
...
Kiri Store, lise öğrencisi tarafından geliştirilmekte ve bakımı yapılmaktadır. Desteğiniz ona şunlarda yardımcı olur:
...
Kiri Store, yalnızca üçüncü taraf geliştiriciler tarafından GitHub'da zaten yayımlanmış olan sürüm dosyalarını keşfetmenize ve indirmenize yardımcı olur.  
...
Kiri Store'u kullanarak, indirilen herhangi bir yazılımı kendi riskinizle kurduğunuzu ve çalıştırdığınızı anlıyor ve kabul ediyorsunuz.  
...
Kiri Store **Apache Lisansı, Sürüm 2.0** kapsamında yayımlanacaktır.

...
Kiri Store jest niezależnym projektem open source, niezwiązanym z GitHub, Inc.
...
> [!IMPORTANT]
> **Użytkownicy macOS:** Możesz zobaczyć ostrzeżenie, że Apple nie może zweryfikować Kiri Store. Dzieje się tak, ponieważ aplikacja jest dystrybuowana poza App Store i nie jest jeszcze notaryzowana. Zezwól na nią w Ustawienia systemowe → Prywatność i bezpieczeństwo → Otwórz mimo to.
...
- **Zarządzanie aplikacjami**
    - Otwieraj, odinstalowuj i instaluj starsze wersje aplikacji bezpośrednio z Kiri Store.
...
## 🔍 Jak moja aplikacja pojawia się w Kiri Store?

Kiri Store nie korzysta z żadnego prywatnego indeksowania ani ręcznych zasad kuracji.
...
2. **Pliki instalacyjne w najnowszym wydaniu**
    ...
    - Kiri Store ignoruje automatycznie generowane archiwa kodu źródłowego (`Source code (zip)` /
      `Source code (tar.gz)`).
...
Jeśli Twoje repozytorium spełnia te warunki, Kiri Store może je znaleźć przez wyszukiwanie i wyświetlić
automatycznie, bez konieczności ręcznego zgłaszania.

---

## 🧭 Jak działa Kiri Store (przegląd)
...
## ✅ Zalety / Dlaczego warto używać Kiri Store?
...
- **Wie, co zainstalowałeś**
  Śledzi aplikacje zainstalowane przez Kiri Store (Android) i podkreśla, gdy dostępne są nowe wydania, abyś mógł je zaktualizować bez ponownego przeszukiwania GitHuba.
...
## 🔐 Certyfikat podpisu APK Kiri Store

Wszystkie oficjalne wydania Kiri Store są podpisane następującym odciskiem certyfikatu:
...
| Pole                           | Wartość                                     |
| ------------------------------ | ------------------------------------------- |
| **Application name**           | Dowolna nazwa (np. *Kiri Store Dev*)      |
...
**Kiri Store** osiągnął **ponad 48 000 aktywnych użytkowników** i **ponad 5 500 gwiazdek na GitHubie** — i jest **w 100% darmowy**, bez reklam, bez śledzenia i bez funkcji premium.
...
Kiri Store jedynie pomaga odkrywać i pobierać pliki wydań, które są już opublikowane na
...
Korzystając z Kiri Store, rozumiesz i akceptujesz, że instalujesz i uruchamiasz jakiekolwiek pobrane oprogramowanie
na własne ryzyko.
...
Kiri Store jest dystrybuowany na warunkach **Licencji Apache, wersja 2.0**.
